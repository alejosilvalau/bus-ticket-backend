    ...
    public static Specification<Location> build(SearchLocationDTO criteria) {
        return (root, query, cb) -> {
            ...
            criteria.cityName().ifPresent(value ->
                    predicates.add(cb.like(cb.lower(root.get("cityName")), StringSearchUtils.likePattern(value))));
            criteria.state().ifPresent(value ->
                    predicates.add(cb.like(cb.lower(root.get("state")), StringSearchUtils.likePattern(value))));
            criteria.postalCode().ifPresent(value ->
                    predicates.add(cb.like(cb.lower(root.get("postalCode")), StringSearchUtils.likePattern(value))));
